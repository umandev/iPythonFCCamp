...
def customer_record():
    conn = mysql.connector.connect(
        host='localhost', database='banksys', user='root', password='himuchi1')
    cursor = conn.cursor()
...
def deposit_amount():
    ...
    if result[0] == 'active':
        ...
        sql2 = 'insert into transaction(amount,type,acno,dot) values(' + amount + ',"deposit",' + acno + ',"' + str(
            today) + '");'
        cursor.execute(sql2)
        cursor.execute(sql1)
        # print(sql1)
...
def withdraw_amount():
    conn = mysql.connector.connect(
        host='localhost', database='bank', user='root', password='himuchi1')
    cursor = conn.cursor()
    clear()
    acno = input('Enter account No :')
    amount = input('Enter amount :')
...
def add_account():
    ...
    aadhar = input('Enter last 4 digits SSN :')
    actype = input('Account Type (saving/current ) :')
    balance = input('Enter opening balance :')
    sql = 'insert into customer(name,address,phone,email,SSN,acc_type,balance,status) values ( "' + name + '","' + addr + '","' + phone + '","' + email + '","' + aadhar + '","' + actype + '",' + balance + ',"active" );'
    # print(sql)
    cursor.execute(sql)
    conn.close()
